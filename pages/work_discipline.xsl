...
<xsl:import href="../utilities/disciplineentries.xsl"/>
...
<xsl:template match="data">
	<div class="span-16">
		<h2>
			<xsl:attribute name="class"><xsl:value-of select="currentdiscipline/entry/title/@handle"/></xsl:attribute>
			<xsl:value-of select="currentdiscipline/entry/title"/>
		</h2>
		<xsl:apply-templates select="disciplinelist/entry"/> <!-- disciplineentries.xsl -->
	</div>
