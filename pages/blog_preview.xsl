	...
	<div class="span-16">
		<xsl:choose>
			<xsl:when test="$is-logged-in = 'true'">
				...
				<h2 class="blue"><xsl:value-of select="blogpreview/entry/title"/></h2>
				<xsl:copy-of select="blogpreview/entry/body/node()"/>
				<form id="commentform" style="margin-bottom:20px;" method="post" action="" enctype="multipart/form-data">
					...
					<input name="fields[post]" type="hidden" value="{$ds-blogpreview}" />
			...
			<xsl:otherwise>
				<h2 class="blue">nothing to see here, yo!</h2>
			</xsl:otherwise>
		</xsl:choose>
	</div>
...
<xsl:template match="events/comments">
	...
	<script type="text/javascript">
	  <xsl:comment>
	      $(document).ready(function(){
	      $('.success, .error').css({'position':'absolute','top':'110px', 'z-index':'9999', 'font-size':'100px', 'border-color':'#FF929B', 'background':'#FF92BB', 'color':'#fff', 'text-shadow':'none', 'padding':'30px'});
	      $('.error').css({'left':'37%'});
	      $('.success').css({'left':'30%'});
		      setTimeout(function(){
		         $('.error').fadeOut(2000);
